...
export default async function ZapatillasAdidasPage() {
  const [sneakers, config] = await Promise.all([
    getProducts({ brand: 'ADIDAS', categorySlug: 'zapatillas' }),
    getSiteConfig('brandSneakersAdidas'),
  ])
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-[13px] font-black uppercase tracking-[0.22em]">Zapatillas Adidas</h1>
      </div>
      <p className="text-[12px] text-muted -mt-4 uppercase tracking-[0.12em]">
        Elegí las zapatillas Adidas que aparecen en la sección destacada de la brand page.
      </p>

      <BrandSneakersConfigEditor
        brand="ADIDAS"
        configKey="brandSneakersAdidas"
        products={sneakers}
