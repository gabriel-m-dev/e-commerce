...
export default function CreateProductForm({ categories, shippingMethods }: CreateProductFormProps) {
  ...
  const [selectedCategorySlugs, setSelectedCategorySlugs] = useState<string[]>(
    categories[0]?.slug ? [categories[0].slug] : []
  )
  ...
  function toggleCategorySlug(slug: string) {
    setSelectedCategorySlugs((prev) => {
      if (prev.includes(slug)) {
        ...
        // If we deselected the primary, pick new primary from remaining
        ...
        // Auto-assign primary if none set yet
  ...
  return (
    <>
      ...
      <button
        ...
        className="border border-foreground px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-foreground transition-colors hover:bg-foreground hover:text-background"
      >
        Agregar producto
      </button>
                  ...
                  {form.images.map((img, i) => (
                    <div key={i} className="flex flex-col gap-1.5">
                      <div className="flex gap-2">
                        ...
                        <label
                          className={[
                            'flex h-[38px] cursor-pointer items-center border border-border px-3 text-[9px] font-semibold uppercase tracking-[0.15em] text-muted transition-colors hover:border-foreground hover:text-foreground shrink-0',
                            uploadingIndices.has(i) ? 'opacity-40 pointer-events-none' : '',
                          ].join(' ')}
