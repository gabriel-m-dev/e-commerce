...
describe('POST /api/checkout/create-preference', () => {
  ...
  it('creates order in DB and returns preferenceId when cart is valid', async () => {
    ...
    // orderId is generated by crypto.randomUUID() in the route — just check it's a non-empty string
    ...
    expect(orderCreateCall.data.shippingMethodId).toBe(SHIPPING_METHOD_ID)
  })
  ...
  it('returns 500 when order creation throws', async () => {
    ...
    mockPreferenceCreate.mockResolvedValue({
      id: 'pref-err',
      init_point: 'https://mp.com/pref-err',
    })
  ...
  it('ignores client-sent price and uses DB price for order total', async () => {
    ...
    // Total = subtotal (15000) + shipping from DB method (16000)
    expect(Number(orderCreateCall.data.total)).toBe(15000 + SHIPPING_METHOD_PRICE)
  })
  ...
  it('returns 400 when shippingMethodId is missing', async () => {
    ...
    const req = makeCreatePreferenceRequest({
      ...
      buyer: makeBuyer(),
      shipping: makeShipping(),
      // shippingMethodId intentionally omitted
    })
    const res = await createPreferencePost(req)
    expect(res.status).toBe(400)
  })
  ...
  it('returns 400 when shippingMethod does not exist', async () => {
    ...
    const req = makeCreatePreferenceRequest({
      ...
      buyer: makeBuyer(),
      shipping: makeShipping(),
      shippingMethodId: 'nonexistent-method-id',
    })
    const res = await createPreferencePost(req)
    expect(res.status).toBe(400)
  })
...
describe('POST /api/webhook/mercadopago', () => {
  ...
  it('sets order to PROCESSING and decrements stock when payment is approved', async () => {
    ...
    // Verify the first arg to $transaction contains an order.update call with CONFIRMED
